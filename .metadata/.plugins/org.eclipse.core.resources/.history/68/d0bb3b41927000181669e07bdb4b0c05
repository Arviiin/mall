...
var E3 = {
    ...
    initItemCat : function(data){
    	...
    	$(".selectItemCat").each(function(i,e){
    		var _ele = $(e);//将原始js转成jQuery对象，命名规则注意一下
    		if(data && data.cid){
    			_ele.after("<span style='margin-left:10px;'>"+data.cid+"</span>");
    		}else{
    			_ele.after("<span style='margin-left:10px;'></span>");
    		}
    		_ele.unbind('click').click(function(){//如果原来已经被绑定别的事件则点击会都执行，为了保证点击只有一个事件，这里解绑再绑定
    			$("<div>").css({padding:"5px"}).html("<ul>")
    			.window({//初始化了一个window，相当于打开了一个窗口，这是一个easyUi的窗口
    				width:'500',
    			    height:"450",
    			    modal:true,//模态，就是窗口出来时候，后面变灰色且不可选
    			    closed:true,
    			    iconCls:'icon-save',
    			    title:'选择类目',
    			    onOpen : function(){//on开头都是事件，这里就是打开
    			    	var _win = this;
